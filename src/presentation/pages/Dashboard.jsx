...
import { listMyPools } from "../../infra/http/pools";   // 👈 NUEVO
...
export default function Dashboard({ thVersion = 0 }) {
  ...
  const alerts = [
    ...(s.ph !== "ok" ? [{
      ...
      msg: `pH: ${data.ph}. ${
        s.ph === "danger" ? "Agregue regulador alcalino." : "Revise y ajuste pH."
      }`,
    ...
    ...(s.cl !== "ok" ? [{
      ...
      msg: `Cloro libre: ${data.cl} ppm. ${
        s.cl === "danger" ? "Agregue cloro." : "Revise y ajuste cloro."
      }`,
      time: history.at(-1)?.time || ""
    }] : []),
    ...
    ...(s.t !== "ok" ? [{
      ...
      msg: `Temperatura: ${data.t} °C. ${
        s.t === "danger" ? "Revise urgentemente la climatización." : "Ajuste el control de temperatura."
      }`,
      time: history.at(-1)?.time || ""
    }] : []),
  ];
  ...
  async function simulate() {
    if (!activePoolId) return;
    const r = await usecases.simulateReading(activePoolId); // 👈 ahora por pileta
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex flex-col">
          ...
          {currentPool ? (
            <span className="text-sm font-medium text-slate-800">
              {currentPool.name} {currentPool.estadoPileta ? `(${currentPool.estadoPileta})` : ""}
            </span>
          ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <ActionConfirm
          onConfirm={(text) => {
            console.log("Acción correctiva registrada:", text, "para pileta", activePoolId);
            // acá más adelante podés mandar al backend con poolId
          }}
        />
      </div>
    </div>
